public class Timer {
    ...
    private static long toMillis(long d) {
        return d / (C2 / C0);
    }
    ...
    public static void computeStats(Timer[] timers, double[] meanDurationsNanos, double[] meanPercentOfTotal, double[] stdDevNanos) {
        ...
        if (meanDurationsNanos == null) {
            meanDurationsNanos = new double[numValues];
        } else {
            for (int i = 0; i < meanDurationsNanos.length; i++)
                meanDurationsNanos[i] = 0.0;
        }
        if (meanPercentOfTotal == null) {
            meanPercentOfTotal = new double[numValues];
        } else {
            for (int i = 0; i < meanPercentOfTotal.length; i++)
                meanPercentOfTotal[i] = 0.0;
        }
        if (stdDevNanos == null) {
            stdDevNanos = new double[numValues];
        } else {
            for (int i = 0; i < meanPercentOfTotal.length; i++)
                meanPercentOfTotal[i] = 0.0;
        }
