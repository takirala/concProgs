...
public class HW3TestGray {
    ...
    static int WARMUPREPS = 0; // default value may be changed with command line
    // vm argument
    static int REPS = 1; // default value may be changed with command line vm
    // argument
    ...
    public static void setUpBeforeClass() throws Exception {
        ...
        if (sourceImageFilename == null)
            System.out.println(
                    "Provide image filename as system property on command " +
                            "line: -DsourceImageFilename" +
                            "=\"C:\\Path\\to\\Pictures\\hw3photo.jpg\"");
        ...
        WARMUPREPS = warmupReps != null ? Integer.parseInt(System
                .getProperty("warmupReps")) : WARMUPREPS;
        String reps = System.getProperty("reps");
        REPS = reps != null ? Integer.parseInt(System.getProperty("reps")) :
                REPS;

        BufferedImage source = sourceImage;
        BufferedImage newImage = new BufferedImage(source.getWidth(), source
    ...
    public void testGray_SS() throws IOException {
        ...
        double[] meanDuration = new double[1];
        //passed as one-element array  so it can be set in statsToString.
...
//				System.out.println(timers[rep].toString());
    ...
    public void testGray_SS_FJ() throws IOException {
        ...
        System.out.println("writing file " + filename);
        File output = new File(filename);
        FJBufferedImage source =
                FJBufferedImage.BufferedImageToFJBufferedImage(sourceImage);
        FJBufferedImage newImage = new FJBufferedImage(source.getWidth(),
